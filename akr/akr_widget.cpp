...
void akr_widget::select(int id, const QVector<int> &value){
    ...
    case 0x203:
    {
        if(value.at(0) == 0b00001)
            diaposon_line->setText("Неисправность");
        
        else if(value.at(0) == 0b00010)
            diaposon_line->setText("РД");
        
        else if(value.at(0) == 0b00100)
            diaposon_line->setText("ПД");
        else if(value.at(0)== 0b01000)
            diaposon_line->setText("СКП");
        else if(value.at(0) == 0b10000)
    ...
    case 0x206:{
        if(!toggled){
            ...
            // set discret
            int diap = value.at(4) & 0xF;
            int puu =  (value.at(4) >> 4 )& 0xF;
            int sred = value.at(6) & 0xF;
            int pd2rd1 = value.at(5) & 0xF;
            int pd1pd2 = (value.at(5) >> 4) & 0xF;
            int pd2id = (value.at(6) >>4 ) & 0xF;
...
float akr_widget::toFloat(const QVector<int> &value){
